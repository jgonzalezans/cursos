...
public class CovidEuropaService {
	...
	public int totalAcumuladosEuropa() throws InterruptedException, ExecutionException{
		
		ExecutorService executor=Executors.newCachedThreadPool();
		//Total de acumulados de "Europa"

		Callable<Integer> tareaAcumulados = ()-> Arrays.stream(items) //Stream<Item>
				.parallel()
				.filter(it->it.getIndicador().equals("cases")&&it.getContinente().equals("Europe"))  //Stream<Item> -solo con los casos-
				.max((it1,it2)->Utilidades.convertirTextAFecha(it1.getFecha()).compareTo(Utilidades.convertirTextAFecha(it2.getFecha())))   //Optional<Item>
				.orElseGet(()->new Item()) //Item
				.getAcumulados();
		
		Future<Integer> future=executor.submit( tareaAcumulados	); 
